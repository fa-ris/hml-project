...
class CifarClient(fl.client.Client):
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        if (p != self.p):
            ...
            self.model = torch.quantization.quantize_dynamic(self.model,{nn.Conv2d, nn.MaxPool2d, nn.ReLU}, dtype=torch.qint8)
            #original_size = os.path.getsize('model.p')
            # print(f"Original Model Size: {original_size} bytes")
            # model_new = torch.quantization.quantize_dynamic(self.model,{nn.Conv2d, nn.MaxPool2d, nn.ReLU, nn.Linear}, dtype=torch.qint8)
